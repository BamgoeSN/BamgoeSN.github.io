...
다항식을 다항식으로 나눈 형태의 식을 **유리식**이라고 부르는데, 유리식은 단일 다항식으로 전개할 수 있습니다.
이때 이 유리식이 약분되어 차수가 유한한 단일 다항식이 되지 못한다면, 대신 차수가 *무한한* 다항식으로 전개할 수 있습니다.

예를 들어, 유리식 $1 / (1-x)$은 약분하여 다항식으로 만드는 게 불가능합니다. 한편, $|x| < 1$이면 다음 식이 성립합니다.
$$ \frac{1}{1-x} = 1 - x + x^2 - x^3 + x^4 + \cdots $$
좌변은 차수가 유한한 유리식이고, 우변은 차수가 무한한 다항식입니다. 즉, 이는 유한차 유리식 $1/(1-x)$을 무한차 다항식 $1-x+x^2-\cdots$으로 전개되었다고 할 수 있습니다.

다시 기존의 문제 상황을 보면, $P(x)$와 $Q(x)$는 각각 유한차 다항식이므로 $P(x)/Q(x)$는 유한차 유리식입니다. 따라서 이는 무한차 다항식이라 생각할 수 있고, 그 식의 $n$차항을 구해야 한다는 말도 자연스러워집니다.
...
여기서 절반 정도로 줄어든 차수를 새로운 $n$으로 잡고, 유리식의 분모 분자를 각각 새로운 $Q(x)$, $P(x)$로 잡으면, 문제의 차수가 절반 정도로 줄어들게 됩니다.
즉, 목표를 달성했습니다.
