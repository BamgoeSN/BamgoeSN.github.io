...
고속 푸리에 변환(fast Fourier transform, FFT)는 길이 $N$인 수열 $A$의 DFT $\tilde{A}$를 $O(N \log N)$의 시간복잡도로 계산할 수 있는 알고리즘이다.
...
한편, 위 그림에서 $R$ 배열을 합쳐서 $\tilde{A}$를 만들어내는 과정으로 두 가지가 널리 알려져 있다. 하나는 DIT(decimation in time)이고, 다른 하나는 DIF(decimation in frequency)이다.
